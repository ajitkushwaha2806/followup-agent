  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-zinc-50 dark:bg-zinc-900/60 p-3.5 rounded-2xl border border-zinc-200 dark:border-zinc-800">
      <div className="relative w-full sm:w-96">
        ...
        <input
          type="text"
          placeholder="Search by account name or cookie..."
          value={searchQuery}
      ...
      <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
        ...
        <button
          onClick={onRefresh}
          title="Refresh list"
          className="p-2 rounded-xl bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 border border-zinc-200 dark:border-zinc-700 transition-all ml-auto cursor-pointer"
        >
          <RefreshCw className={`w-4 h-4 ${isFetching ? "animate-spin text-red-500" : ""}`} />
        </button>
      </div>
    </div>
  );
